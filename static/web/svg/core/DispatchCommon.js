...
var SELECTION_MIRRORING_ON = "Selection Mirroring: On";
var SELECTION_MIRRORING_OFF = "Selection Mirroring: Off";
var MIRRORED_TRAIN_SELECTION = "MIRRORED_TRAIN_SELECTION";
var SELECTION_MIRRORING_TEXTID = "SelectionMirroring";
...
function setDispatchAuthorization(name, route, state, trainID)
{
    ...
    // Re-evaluate the unauthorized-occupancy alarm for this object after its
    // state has been applied. Guarded inside evaluateAlarm so it can never
    // disrupt dispatch rendering.
    evaluateAlarm(name);
}
...
function evaluateAlarm(name)
{
    ...
    {
        console.log("evaluateAlarm error for " + name + ": " + e);
    }
}

/* alarmBlockElements([String] num) -> [Array of SVGElement]
 * The block segment + its A/B/... sub-segments for a block number.
 */
function alarmBlockElements(num)
{
    var els = [];
    var base = svgDocument.getElementById(DISPATCHSEGMENT_OBJID_PREFIX + num);
    if(base != null) els.push(base);
    var suffix = 'A';
    var seg = svgDocument.getElementById(DISPATCHSEGMENT_OBJID_PREFIX + num + suffix);
    while(seg != null)
    {
        els.push(seg);
        suffix = String.fromCharCode(suffix.charCodeAt(0) + 1);
        seg = svgDocument.getElementById(DISPATCHSEGMENT_OBJID_PREFIX + num + suffix);
    }
    return els;
}
...
function clickTrainListLabel(elemID)
{
	...
	else
	{
        ...
        if((returnedTrainID != null) && (returnedTrainID != ""))
		{
            ...
            setNextAuthorizationTrain(trainIDElemID, true);
		}
	}
}
...
{
    if(getSelectionMirroringEnabled())
        setSVGText(SELECTION_MIRRORING_TEXTID, SELECTION_MIRRORING_OFF);
    else
        setSVGText(SELECTION_MIRRORING_TEXTID, SELECTION_MIRRORING_ON);
}

function setServerMirroredSelection(newSelection)
{
    //alert("setServerMirroredSelection(" + newSelection + ")");

    var tempStateChangeRequests = stateChangeRequests;
    
    stateChangeRequests = new Array();
    
    stateChangeRequests.push(new ServerObject(MIRRORED_TRAIN_SELECTION, SERVER_TYPE_DISPATCH, newSelection));
...
function setDispatchObject(object)
{
    if(object.type == SERVER_TYPE_DISPATCH)
    {
        ...
        if(object.name.search("[N|S|O][1-8]") == 0)
        {
            ...
            if(selectedNSO == object.name.substring(0,2))
            {
                if((object.value == "") || (object.value == null))
                {
                    ...
                    setNextAuthorizationTrain(selectedNSO + "Train", true);
            }
        ...
        // Handle selected train mirroring
        else if(object.name.search(MIRRORED_TRAIN_SELECTION) == 0)
        {
            if(getSelectionMirroringEnabled())
                updateMirroredSelection(object.value);
